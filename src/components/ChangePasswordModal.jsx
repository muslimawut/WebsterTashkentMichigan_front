...
const ChangePasswordModal = ({ isOpen, onClose }) => {
  ...
  const handleSendResetCode = async (e) => {
    ...
    try {
      ...
      showNotification(`Password reset code sent to ${formData.email}`, 'success');
      setStep(2);
    } catch (error) {
      showNotification(error.message || 'Failed to send reset code. Please try again.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResetPassword = async (e) => {
    ...
    if (!formData.activationCode) {
      showNotification('Please enter the activation code', 'warning');
      return;
    }
  ...
  return (
    <>
      <div 
        className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-[150] ${
              ...
              <form onSubmit={handleSendResetCode} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    required
                    disabled={loading}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#024890] focus:border-transparent outline-none transition disabled:opacity-50 bg-gray-50"
