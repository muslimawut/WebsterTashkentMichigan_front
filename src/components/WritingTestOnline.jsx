...
import { useWritingProctoringOnline } from '../hooks/useWritingProctoringOnline';
import {
...
// ── ONLINE Writing test ──
// Metrica imtihoni tugagach ProctoringExam shu sahifaga o'tadi va kamera+ekran
// streamlarini handoff qiladi. Bu yerda proctoring TO'XTAMAYDI: kamera, mikrofon
// va ekran ulashish davom etadi (snapshot + qisqa klip + davriy monitoring).
// Offline /writing-test'ga tegilmagan — bu alohida variant.
...
const WritingTestOnline = () => {
  ...
  // ── Gate (Metrica'dan keyingi "Continue to Writing") ──
  const [gateStep, setGateStep]       = useState(true);
  ...
  // Metrica'dan meros kamera+ekran streamlari — gate ochilgunча ushlab turamiz.
  const heldMediaRef       = useRef(null);
  const mediaConsumedRef   = useRef(false);

  // ── Meros media'ni mount'da olamiz (bridge 1s ichida bo'shatib yubormasin) ──
  useEffect(() => {
    cancelProctorMediaRelease();
    heldMediaRef.current = takeProctorMedia(); // {cameraStream, screenStream} | null
    return () => {
      // Agar student gate'dan Begin bosmasdan chiqib ketsa — meros media'ni to'xtatamiz.
      if (!mediaConsumedRef.current && heldMediaRef.current) {
        heldMediaRef.current.cameraStream?.getTracks?.().forEach((t) => t.stop());
        heldMediaRef.current.screenStream?.getTracks?.().forEach((t) => t.stop());
        heldMediaRef.current = null;
      }
    };
  }, []);

  // ── Begin (user gesture: fullscreen + media + writing session) ──
  const handleBegin = async () => {
    if (!fullName.trim() || !passportId.trim()) {
      setStartError('Full name and passport ID are required.');
      return;
    }
    setStartLoading(true);
    setStartError('');

    // Fullscreen'ni AWAIT'lardan oldin, gesture saqlanib turganda so'raymiz.
    try {
    ...
    try {
      const candidate = { fullName: fullName.trim(), passportId: passportId.trim() };
      const held = heldMediaRef.current || {};
      // Online monitoring: meros streamlarni SAQLAB davom ettiradi (yo'q bo'lsa
      // getDisplayMedia/getUserMedia bilan shu gesture ichida so'raydi).
      await startMonitoring({
        fullName: candidate.fullName,
        passportId: candidate.passportId,
        cameraStream: held.cameraStream || null,
        screenStream: held.screenStream || null,
      });
      mediaConsumedRef.current = true; // media endi hook nazoratida
      ...
      if (status === 403) {
        ...
        setStartError(err?.message || monitoringError || 'Could not start the online Writing section. Please try again.');
      }
    } finally {
  ...
  // Count words and characters + keep ref in sync
  useEffect(() => {
  ...
  // Copy / cut / paste ni BUTUN SAHIFA (document) darajasida bloklaymiz — nafaqat
  // textarea'da. Shunday qilib boshqa page/ilovadan nusxa olib paste qilish ham,
  // istalgan joyda copy/cut ham to'siladi (drag-drop matn ham).
  useEffect(() => {
    if (gateStep) return; // yozuv boshlanmagunча kerak emas
    ...
    const preventAction = (e) => {
      ...
      const eventTypes = {
        ...
        drop: ['paste_attempt', 'Drag-drop text attempt during Writing'],
        dragover: [null, null],
      };
      const violation = eventTypes[e.type];
      if (violation && violation[0]) reportViolation(violation[0], violation[1]);
      return false;
    };

    const preventKeyboardShortcuts = (e) => {
      const key = (e.key || '').toLowerCase();
      // Ctrl/Cmd + C/X/V va Insert-variantlari (Shift+Insert = paste, Ctrl+Insert = copy)
      const comboCopyCutPaste = (e.ctrlKey || e.metaKey) && ['c', 'x', 'v'].includes(key);
      const insertPaste = e.shiftKey && key === 'insert';
      const insertCopy = (e.ctrlKey || e.metaKey) && key === 'insert';
      if (comboCopyCutPaste || insertPaste || insertCopy) {
        e.preventDefault();
        e.stopPropagation();
        const label = (key === 'x') ? ['cut_attempt', 'Cut keyboard shortcut during Writing']
          : (key === 'c' || insertCopy) ? ['copy_attempt', 'Copy keyboard shortcut during Writing']
          : ['paste_attempt', 'Paste keyboard shortcut during Writing'];
        reportViolation(label[0], label[1]);
        return false;
      }
    };

    // capture: true — hodisa nishonga yetib bormasdan turib to'xtatamiz.
    const opts = { capture: true };
    document.addEventListener('copy', preventAction, opts);
    document.addEventListener('cut', preventAction, opts);
    document.addEventListener('paste', preventAction, opts);
    document.addEventListener('contextmenu', preventAction, opts);
    document.addEventListener('drop', preventAction, opts);
    document.addEventListener('dragover', preventAction, opts);
    document.addEventListener('keydown', preventKeyboardShortcuts, opts);

    return () => {
      document.removeEventListener('copy', preventAction, opts);
      document.removeEventListener('cut', preventAction, opts);
      document.removeEventListener('paste', preventAction, opts);
      document.removeEventListener('contextmenu', preventAction, opts);
      document.removeEventListener('drop', preventAction, opts);
      document.removeEventListener('dragover', preventAction, opts);
      document.removeEventListener('keydown', preventKeyboardShortcuts, opts);
    };
  }, [reportViolation, gateStep]);
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden && testStarted && !isSubmitted && !hasSubmittedRef.current && !isWarningActive) {
        ...
        setMouseLeaveCount(mouseLeaveCountRef.current);
        setShowMouseWarning(true);
        reportViolation('tab_switch', 'Writing tab was hidden');
        if (warningTimeoutRef.current) clearTimeout(warningTimeoutRef.current);
        warningTimeoutRef.current = setTimeout(() => setShowMouseWarning(false), 5000);
        cooldownTimer = setTimeout(() => { isWarningActive = false; }, 3000);
      }
    };
  ...
  // Detect mouse leaving screen / edges in fullscreen
  useEffect(() => {
    let isWarningActive = false;
    let cooldownTimer = null;
  ...
  // ── Gate screen (Continue to Writing) ──
  if (gateStep) {
            ...
            <div className="md:w-5/12 relative overflow-hidden flex flex-col justify-between p-10" style={{ backgroundColor: '#1a3460', minHeight: 480 }}>
              ...
              <div className="relative">
                ...
                <p className="text-sm mb-8" style={{ color: 'rgba(255,255,255,0.55)' }}>
                  Your Metrica exam is complete. The proctored Writing section starts next.
                </p>

                <div className="rounded-xl p-4 mb-2" style={{ backgroundColor: 'rgba(255,255,255,0.08)', border: '1px solid rgba(52,211,153,0.35)' }}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="w-2 h-2 rounded-full bg-green-400 inline-block animate-pulse" />
                    <p className="text-sm font-semibold text-white">Proctoring stays active</p>
                  </div>
                  <p className="text-xs" style={{ color: 'rgba(255,255,255,0.6)' }}>
                    Your camera, microphone and full-screen sharing remain on for the entire Writing section — monitoring does not stop.
                  </p>
                </div>
              </div>
            ...
            <div className="md:w-7/12 flex flex-col justify-center px-10 py-10">
              ...
              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3.5 mb-5">
                <div className="flex items-center gap-2 mb-2.5">
                  ...
                  <p className="text-sm font-bold text-amber-900">Writing section rules</p>
                </div>
                <ul className="space-y-1.5 text-xs leading-relaxed text-amber-900">
                  <li className="flex gap-2"><span className="font-bold">•</span><span>Copy, cut, paste and external text are not allowed.</span></li>
                  <li className="flex gap-2"><span className="font-bold">•</span><span>Stay in fullscreen; do not switch tabs, windows or apps.</span></li>
                  <li className="flex gap-2"><span className="font-bold">•</span><span>Camera, microphone and screen sharing stay recorded — the 60-minute timer cannot be paused.</span></li>
                </ul>
              </div>
              ...
              >
                {startLoading ? (
                  <>
                    ...
                    Preparing writing section...
                  </>
                ) : 'Begin Writing Section'}
              </button>

              <p className="text-center text-xs text-gray-400 mt-3">
                Pressing Begin enters fullscreen. Keep your camera on and stay on this page.
              </p>
            </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3 mb-2">
            ...
            <div className="hidden md:flex items-center gap-2 flex-wrap">
              {[
                { icon: '📷', text: 'Camera on' },
                { icon: '🎙️', text: 'Mic on' },
                { icon: '🖥️', text: 'Screen shared' },
                { icon: '⛔', text: 'No copy-paste' },
                { icon: '🚫', text: 'No tab switch' },
              ].map((rule) => (
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-md transition-all ${
                      layout === key ? 'bg-white shadow text-blue-600' : 'text-gray-400 hover:text-gray-600'
                    }`}
                  >
                    {icon}
                  </button>
          ...
          <div className="mt-4">
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                ...
                style={{ width: `${Math.min((totalTime / 3600) * 100, 100)}%` }}
              />
            </div>
                ...
                <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex-shrink-0">
                  ...
                  <div className="mt-3 px-3 py-2 bg-blue-50 rounded-lg">
                    <p className="text-xs text-blue-700">
                      <strong>Reminder:</strong> Camera &amp; screen recording are active. Copy/paste is disabled.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-10 text-center">
            ...
            <p className="text-gray-600 mb-4">
              You have violated the exam rules. Your essay session has been terminated.
            </p>
...
export default WritingTestOnline;
